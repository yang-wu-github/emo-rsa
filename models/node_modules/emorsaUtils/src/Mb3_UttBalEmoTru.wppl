var Mb3_UttBalEmoTru = function(utterance, emotion, params){
	...
	var speaker1 = cache(function(state, infGoal, socGoal, params) {
	  Infer({model: function(){
	    ...
	    var emotion = uniformDraw(emotions);
			var literalMeaningWeight = params.literalMeanings[utterance][state]
	    var literalEmoUtility = Math.log(literalMeaningWeight);

	    factor( params.speakerOptimality * (speakerUtility + literalEmoUtility)/2);
