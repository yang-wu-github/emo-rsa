...
state <- lm(response ~ utt * exp, data = d2[d$question=="ground truth",])
summary(state)
infGoal <- lm(response ~ utt * exp, data = d2[d$question=="accurate feedback",])
summary(infGoal)
socGoal <- lm(response ~ utt * exp, data = d2[d$question=="niceness",])
summary(socGoal)
```
...
ggsave(here("models/plots_4models.pdf"), width = 15, height = 9)
