...
d.M <- webppl(
  program_file = here("models/TEST2_M00_UttInfNoEmo.wppl"),
  data = df,
  data_var = "df"
)

d.M.l1 <- bind_rows(d.M$l1) %>%
  mutate(model = "M: UttInfNoEmo")
...
### Model 1a1: Emotional expression is social
```{r}
df <- expand.grid(
  utterance = c("good", "bad"),
  emotion = c("smile", "frown")
...
d.M1a1 <- webppl(
  program_file = here("models/TEST2_M1a1_UttInf_EmoSoc.wppl"),
  data = df,
  data_var = "df"
)

d.M1a1.l1 <- bind_rows(d.M1a1$l1) %>%
  mutate(model = "M1a1: UttInfEmoSoc")

# l1: state
M1a1.state <- d.M1a1.l1 %>%
  group_by(utterance, emotion, state, model) %>%
  summarize(expval = sum(prob)) 

# l1: infGoal
M1a1.infGoal <- d.M1a1.l1 %>%
  group_by(utterance, emotion, infGoal, model) %>%
  summarize(expval = sum(prob)) 

# l1: socGoal
M1a1.socGoal <- d.M1a1.l1 %>%
  group_by(utterance, emotion, socGoal, model) %>%
  summarize(expval = sum(prob)) 
```

### Model 1a3: Emotional expression is social
```{r}
df <- expand.grid(
  utterance = c("good", "bad"),
  emotion = c("smile", "frown")
...
d.M1a3 <- webppl(
  program_file = here("models/TEST2_M1a3_Emo_Soc.wppl"),
  data = df,
  data_var = "df"
)

d.M1a3.l1 <- bind_rows(d.M1a3$l1) %>%
  mutate(model = "M1a3: EmoSoc")

# l1: state
M1a3.state <- d.M1a3.l1 %>%
  group_by(utterance, emotion, state, model) %>%
  summarize(expval = sum(prob)) 

# l1: infGoal
M1a3.infGoal <- d.M1a3.l1 %>%
  group_by(utterance, emotion, infGoal, model) %>%
  summarize(expval = sum(prob)) 

# l1: socGoal
M1a3.socGoal <- d.M1a3.l1 %>%
  group_by(utterance, emotion, socGoal, model) %>%
  summarize(expval = sum(prob)) 
```
...
### Model 3: Infer whether emo is communicative (true vs balance)
...
### Model 3a: Infer whether emo is communicative (true vs social)
...
d.M3a <- webppl(
  program_file = here("models/M3a_InferEmoCom.wppl"),
  data = df,
  data_var = "df"
  )

d.M3a.l1 <- bind_rows(d.M3a$l1) %>%
  mutate(model = "M3a: InferEmoTrue/Soc")
...
# l1: emoComm
M3a.emoComm <- d.M3a.l1 %>%
  group_by(utterance, emotion, emoComm, model) %>%
  summarize(expval = sum(prob)) 

# l1: infGoal
M3a.infGoal <- d.M3a.l1 %>%
  group_by(utterance, emotion, infGoal, model) %>%
  summarize(expval = sum(prob)) 

# l1: socGoal
M3a.socGoal <- d.M3a.l1 %>%
  group_by(utterance, emotion, socGoal, model) %>%
  summarize(expval = sum(prob)) 
```
...
```{r}
# state: models
state0 <- rbind(M.state, M0.state, M1.state, M1a1.state, M1a3.state, M2.state, M3a.state, M3.state)
state <- state0 %>%
...
state.total <- state.plot + h.state.plot + plot_layout(ncol=2, widths=c(8,1))
state.total
```
### infGoal
```{r}
# models
infGoal0 <- rbind(M.infGoal, M0.infGoal, M1.infGoal, M1a1.infGoal, M1a3.infGoal, M2.infGoal, M3.infGoal, M3a.infGoal)
infGoal <- infGoal0 %>%
  mutate(condition = paste(utterance, emotion),
         condition = factor(condition, levels = c("good smile", "bad frown", "good frown", "bad smile")))
...
infGoal.total <- infGoal.plot + h.infGoal.plot + plot_layout(ncol=2, widths=c(8,1))
infGoal.total
```
### socGoal
```{r}
# models
socGoal0 <- rbind(M.socGoal, M0.socGoal, M1.socGoal, M1a1.socGoal, M1a3.socGoal, M2.socGoal, M3.socGoal, M3a.socGoal)
socGoal <- socGoal0 %>%
  mutate(condition = paste(utterance, emotion),
         condition = factor(condition, levels = c("good smile", "bad frown", "good frown", "bad smile")))
...
socGoal.total <- socGoal.plot + h.socGoal.plot + plot_layout(ncol=2, widths=c(8,1))
socGoal.total
```
###total
```{r}
total <- state.total / infGoal.total / socGoal.total
ggsave(here("models/TEST2_plots_4models_line_graded_M3a.pdf"), width=15, height=8)
```
...
M1_emoComm_ave <- M3_emoComm_ave %>%
  mutate(model= "M1: EmoComm",
           mean = 1)
M1a1_emoComm_ave <- M3_emoComm_ave %>%
  mutate(model= "M1a1: UttInfEmoSoc",
           mean = 1)
M1a3_emoComm_ave <- M3_emoComm_ave %>%
  mutate(model= "M1a3: EmoSoc",
           mean = 1)
...
h_emoComm_ave <- M3_emoComm_ave %>%
  mutate(model= "human (pilot)",
           mean = NA)
emoComm_ave <- rbind(M_emoComm_ave,M0_emoComm_ave, M1_emoComm_ave, M1a1_emoComm_ave, M1a3_emoComm_ave, M2_emoComm_ave, M3_emoComm_ave, M3a_emoComm_ave, h_emoComm_ave)
emoComm_ave <- emoComm_ave %>%
  mutate(condition = factor(condition, levels = c("good smile", "bad frown", "good frown", "bad smile")),
         model = factor(model, levels = c("M: UttInfNoEmo","M0: NoEmo", "M1: EmoComm", "M1a1: UttInfEmoSoc", "M1a3: EmoSoc", "M2: EmoNotComm", "M3: InferEmoComm", "M3a: InferEmoTrue/Soc", "human (pilot)")))
...
ggsave(here("models/TEST2_plots_4models_line_averaged_xCondition_groupModel_M3a.pdf"), width=13, height=8)
```
